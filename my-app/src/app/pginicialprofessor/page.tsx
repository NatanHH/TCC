...
export default function PageProfessor() {
  ...
  }, [turmaSelecionada, professorId]); // Inclua professorId para garantir reatividade
  ...
  async function aplicarAtividadeEmTurma(idTurma: number) {
    ...
    await fetch("/api/aplicaratividade", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
      ...
      <main className={styles.paginaAlunoMain}>
        ...
        >
          {!turmaSelecionada ? (
            ...
            <>
              <h2 style={{ color: "#fff", margin: "20px 0" }}>
                Atividades já aplicadas na turma {turmaSelecionada.nome}
              </h2>
              {loadingAtividades ? (
        ...
        {/* Modal para aplicar atividade em turma */}
        {modalAplicarAberto && (
          <div className={styles.modal}>
            <div className={styles.modalContent}>
              ...
              {turmas.map((turma) => (
                <button
                  key={turma.idTurma}
                  onClick={() => aplicarAtividadeEmTurma(turma.idTurma)}
                  style={{ margin: 6 }}
                >
                  {turma.nome}
                </button>
              ))}
              <button onClick={fecharModalAplicar} style={{ marginTop: 16 }}>
                Cancelar
              </button>
            </div>
          </div>
        ...
        {/* MODAIS E DEMAIS COMPONENTES, igual ao seu código atual */}
